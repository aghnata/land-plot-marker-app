...
export default function Edit({ plot }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/land-plots/${plot.id}/edit` },
    ];
    ...
    const getMapCenter = (): [number, number] => {
        if (boundaryPoints.length > 0) {
            const avgLat =
                boundaryPoints.reduce((sum, p) => sum + p.lat, 0) / boundaryPoints.length;
            const avgLng =
                boundaryPoints.reduce((sum, p) => sum + p.lng, 0) / boundaryPoints.length;
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 lg:flex-row">
                ...
                <div className="w-full space-y-4 lg:w-96 lg:shrink-0">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Drag the markers on the map to adjust boundary points.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                ...
                <div className="relative min-h-[400px] flex-1 overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
                    <MapContainer center={getMapCenter()} zoom={19}>
                        <PlotPolygon points={boundaryPoints} isSelected />
                        {boundaryPoints.map((point, idx) => (
                            <PlotMarker
                                key={idx}
                                position={point}
                    ...
                    <div className="pointer-events-none absolute left-4 top-4 rounded-lg bg-background/90 px-3 py-2 shadow-lg">
                        <p className="text-sm font-medium">
                            Drag markers to adjust boundary positions
                        </p>
                    </div>
                </div>
            </div>
